...
def in_memory_db():
    ...
    conn.executescript(
        ...
        INSERT INTO shows VALUES (2, 'The Example Show 2', 2001, 'fakeurl.com', '26/02/2000:21:00');
        INSERT INTO shows VALUES (3, 'the Example Show 3', 2002, 'fakeurl.com', '26/02/2000:21:00');
...
def test_retrieve_n_shows_returns_n_shows(in_memory_db):
    results = in_memory_db.retrieve_n_shows('The', 3)
    results_as_tuples = [(row['id'], row['name'], row['tvmaze_id']) for row in results]
    assert results_as_tuples == [(2, 'The Example Show 2', 2001), (3, 'the Example Show 3', 2002)]
...
def test_insert_episode_returns_exception(in_memory_db):
    in_memory_db.insert_episode(1, 2, 'Example Title', '23rd July 2022')
    with pytest.raises(sqlite3.IntegrityError):
        in_memory_db.insert_episode(1, 2, 'Example Title', '23rd July 2022')
