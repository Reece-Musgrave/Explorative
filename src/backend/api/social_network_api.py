...
async def retrieve_followed_shows(user_id: str, db: Session = Depends(get_db)):
    try:
...
async def insert_followed_show(user_id: str, show_id: str, db: Session = Depends(get_db)):
    try:
...
async def delete_followed_show(user_id: str, show_id: str, db: Session = Depends(get_db)):
    try:
...
async def retrieve_followed_users(user_id: str, db: Session = Depends(get_db)):
    try:
...
async def insert_followed_user(user_id: str, target_user_id: str, db: Session = Depends(get_db)):
    try:
        return create_follow_user_relationship(db, user_id, target_user_id)
    except IntegrityError:
        raise HTTPException(status_code=409)

@router.delete('/api/v1/social/followed_user', response_model=MessageResponse)
async def delete_followed_user(user_id: str, target_user_id: str, db: Session = Depends(get_db)):
    try:
        return delete_follow_user_relationship(db, user_id, target_user_id)
    except NotFoundError:
        raise HTTPException(status_code=404)