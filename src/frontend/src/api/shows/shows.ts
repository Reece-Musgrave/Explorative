...
export async function retrieveShow(showName: string): Promise<RetrieveShowOutput> {
    const responseShowCall = await fetch(`${API_BASE_URL}/api/v1/database/retrieve-show/${encodeURIComponent(showName)}`);
    if (responseShowCall.ok) {
        const { id, name, maze_id, url }= await responseShowCall.json();
        const responseSeasonCall = await fetch (`${API_BASE_URL}/api/v1/database/retrieve-season/${id}`);
        ...
        const responseMazeCall = await fetch(`${API_BASE_URL}/api/v1/showapi/retrieve-show/${encodeURIComponent(showName)}`);
        if (responseMazeCall.ok) {
            const { id, name, poster_url } = await responseMazeCall.json();
            const responseSeasonCall = await fetch(`${API_BASE_URL}/api/v1/showapi/retrieve-seasons/${id}`);
            if (responseSeasonCall.ok) {
                const seasonCount: number = await responseSeasonCall.json();
                const episodeHolder: [number, number, number][] = [];
                for (let i = 1; i  <= seasonCount; i++) {
                    const episodeResponse = await fetch(`${API_BASE_URL}/api/v1/showapi/retrieve-number-episodes/${id}/${i}`);
...
async function insertShow(showName: string, maze_id: number, url: string, seasons: number, episodes: [number, number, number][]): Promise<void> {
    ...
    const responseInsertShow = await fetch(`${API_BASE_URL}/api/v1/database/insert-show`, {
        method: "PUT",
        headers: {
    ...
    if (responseInsertShow.status === 204) {
        ...
        for (let i = 1; i  <= seasons; i++) {
            ...
            const responseInsertSeason = await fetch(`${API_BASE_URL}/api/v1/database/insert-season`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json"
                },
            ...
            const responseSeasonFetch = await fetch(`${API_BASE_URL}/api/v1/database/retrieve-season/${id}`);
            ...
            if(responseInsertSeason.status === 204) {
                for (let j = 1; j <= episodeTuple![2]; j++){
                    const responseEpisodeData = await fetch(`${API_BASE_URL}/api/v1/showapi/retrieve-episodes/${maze_id}/${i}/${j}`);
                    if (responseEpisodeData.ok){
                        ...
                        const responseInsertEpisode = await fetch(`${API_BASE_URL}/api/v1/database/insert-episode`, {
                            method: "PUT",
                            headers: {
                                "Content-Type": "application/json"
                            },
